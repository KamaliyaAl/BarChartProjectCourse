...
class TestCase(unittest.TestCase):
    ...
    @patch('task.load_the_data')
    @patch('task.required_columns')
    @patch('task.interested_platforms')
    @patch('task.group_data')
    @patch('task.bar_chart_details')
    @patch('task.set_positions')
    @patch('task.bar_settings')
    @patch('matplotlib.pyplot.show')
    def test_show_bar(self, mock_show, mock_bar_settings, mock_set_positions, mock_bar_chart_details, mock_group_data,
                      mock_interested_platforms, mock_required_columns, mock_load_the_data):
        # Подготовка данных
        mock_load_the_data.return_value = pd.DataFrame({'name': ['Game1'], 'platform': ['PS4'], 'genre': ['Action']})
        df = mock_load_the_data(self.filename)

        # Настройка возврата для bar_chart_details
        mock_bar_chart_details.return_value = (0.5, range(10), ['PS4'], ['Action'], ['red'])

        show_bar(df)

        # Проверка вызовов
        mock_required_columns.assert_called_once()
        mock_interested_platforms.assert_called_once()
        mock_group_data.assert_called_once()
        mock_bar_chart_details.assert_called_once()
        mock_set_positions.assert_called_once()
        mock_bar_settings.assert_called_once()
